...
$password = $mysqli->escape_string(password_hash($_POST['password'].$salt, PASSWORD_BCRYPT));
$hash = $mysqli->escape_string( md5( rand(0,1000) ) );

// Check if user with that email already exists
$result = $mysqli->query("SELECT * FROM customers_information WHERE email='$email'") or die($mysqli-error());
   ...
   // Email doesn't exist in a database, proceed to create account
{
	...
	$sql = "INSERT INTO customers_information (id, first_Name, last_Name, gender, age, birthday, address, contact, email, password, hash, salt, role)"
		. "VALUES (NULL, '$first_Name', '$last_Name', '$gender', '
		$age', '$birthday', '$address', '$contact', '$email', '$password', '$hash', '$salt', '$role')";
